...
node_pairs = get_node_pairs(df.user.unique())
...
link_list = [node_pair + (get_linkweight_per_user_pair(node_pair, df),) for node_pair in node_pairs]

with open('{}userNodes_link_list.pickle'.format(fpath.split('.csv')[0]), 'wb') as f:
